...
public class ReferenceDrawer : PropertyDrawer
{
    ...
    public ScriptableObject CreateNew(SerializedProperty property)
    {
        ...
        dest = EditorUtility.SaveFilePanel("Save object as", dest, "New " + type.ToString(), "asset");

        if (!string.IsNullOrEmpty(dest) && TryMakeRelative(Path.GetDirectoryName(Application.dataPath), dest, out dest))
        {
    ...
    private bool TryMakeRelative(string absoluteParentPath, string absolutePath, out string relativePath)
    {
        ...
        if (!absolutePath.StartsWith(absoluteParentPath))
            return false;

        relativePath = absolutePath.Replace(absoluteParentPath, "");
